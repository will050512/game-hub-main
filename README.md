...
### v1.1.3 (2026-06-11)
- **響應式優化** - 全頁面 UI 重疊檢查與修復，橫豎屏適配（Hero 區支援橫屏手機、卡片縮小至 240px）
- **觸控靈敏度** - 暫停按鈕從 38px 放大至 44px（符合 WCAG 最小觸控目標）、虛擬按鈕從 64px 提升至 72px
- **觸控修正** - Canvas 觸控追蹤從 `touches[0]` 改為 `changedTouches[0]`，多指觸控更準確
- **商店版面** - 手機版側邊面板移至商品列表下方，避免遮擋主要內容
- **卡片排版** - 小螢幕手機（≤480px）遊戲卡片縮短至 240px、縮圖比例 16:10，避免一屏只能放一張卡

### v1.1.2 (2026-06-11)
- **Bug 修復** - Tower Defense 遊戲結束畫面按鈕渲染修復（`roundRect` 缺少 `ctx.beginPath()`）
- **Bug 修復** - Sudoku 遊戲結束畫面按鈕渲染修復（`roundRect` 缺少 `ctx.beginPath()`）
- **內容增加** - 數獨新增 2 個簡單難度、1 個困難難度謎題
- **全面審核** - 12 款遊戲完整代碼審查，確認無排版重疊、邏輯錯誤
